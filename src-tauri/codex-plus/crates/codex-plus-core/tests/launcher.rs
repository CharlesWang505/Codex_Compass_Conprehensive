...
use codex_plus_core::launcher::{
    CodexLaunch, DefaultLaunchHooks, LaunchHooks, LaunchOptions, LocalHelperServer,
    MacosCleanupPolicy, build_codex_arguments, build_codex_arguments_for_settings,
...
#[tokio::test]
async fn local_helper_server_exposes_health_and_stops_cleanly() {
    use tokio::io::{AsyncReadExt, AsyncWriteExt};

    let probe = std::net::TcpListener::bind(("127.0.0.1", 0)).unwrap();
    let port = probe.local_addr().unwrap().port();
    drop(probe);
    let server = LocalHelperServer::default();
    ...
    assert!(response.contains("后端已连接"));
    server.stop().await;
    assert!(!server.is_running().await);
}

#[tokio::test]
async fn local_helper_server_reuses_healthy_existing_endpoint() {
    let probe = std::net::TcpListener::bind(("127.0.0.1", 0)).unwrap();
    let port = probe.local_addr().unwrap().port();
    drop(probe);
    let owner = LocalHelperServer::default();
    let observer = LocalHelperServer::default();

    owner.start("127.0.0.1", port).await.unwrap();
    observer.start("127.0.0.1", port).await.unwrap();

    assert!(owner.is_running().await);
...
#[tokio::test]
async fn launch_starts_8787_helper_when_hot_switch_is_enabled() {
    let temp = tempfile::tempdir().unwrap();
    let app_dir = temp.path().join("Codex.app");
    std::fs::create_dir_all(&app_dir).unwrap();
    let status_store = StatusStore::new(temp.path().join("latest-status.json"));
    ...
    let before_stop = events.lock().unwrap().clone();
    assert!(before_stop.contains(&"start-helper:8787".to_string()));
    assert_eq!(handle.helper_port, 8787);

    handle.wait_for_codex_exit().await.unwrap();
    let after_stop = events.lock().unwrap().clone();
    assert!(after_stop.contains(&"shutdown-helper:8787".to_string()));
}
