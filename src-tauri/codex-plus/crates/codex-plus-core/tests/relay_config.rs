...
use codex_plus_core::model_suffix::ModelCatalogEntry;
use codex_plus_core::relay_config::{
    apply_hot_switch_config_to_home, apply_pure_api_config_to_home,
    apply_relay_config_file_to_home, apply_relay_config_to_home,
    apply_relay_config_to_home_preserving_existing_with_protocol, apply_relay_files_to_home,
...
#[test]
fn hot_switch_relay_config_preserves_existing_context_sections() {
    let temp = tempfile::tempdir().unwrap();
    std::fs::write(
        temp.path().join("config.toml"),
        r#"model = "gpt-5"
...
"#,
    ...
    let result = apply_relay_config_to_home_preserving_existing_with_protocol(
        temp.path(),
    ...
    assert!(updated.contains(r#"base_url = "http://127.0.0.1:8787/v1""#));
    assert!(updated.contains(r#"experimental_bearer_token = "codex-plus-hot-switch""#));
}

#[test]
fn hot_switch_model_routing_writes_catalog_and_preserves_existing_context() {
    let temp = tempfile::tempdir().unwrap();
    std::fs::write(
        temp.path().join("config.toml"),
        "model = \"old-model\"\n\n[mcp_servers.context7]\ncommand = \"npx\"\n",
    ...
    let catalog =
        std::fs::read_to_string(temp.path().join("model-catalogs").join("hot-switch.json"))
            .unwrap();
    assert!(config.contains("[mcp_servers.context7]"));
    assert!(config.contains(r#"model = "model-a""#));
    assert!(config.contains(r#"model_catalog_json = "model-catalogs/hot-switch.json""#));
    assert!(catalog.contains(r#""slug": "model-a""#));
    assert!(catalog.contains(r#""slug": "model-b""#));
