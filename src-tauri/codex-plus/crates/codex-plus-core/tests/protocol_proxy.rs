...
use codex_plus_core::settings::{
    ...
    HotSwitchModelMapping, RelayMode, RelayProfile,
};
use serde_json::json;
...
#[tokio::test]
async fn hot_switch_model_mapping_selects_mapped_key_and_rewrites_upstream_model() {
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
        .await
        .unwrap();
    let addr = listener.local_addr().unwrap();
    let server = tokio::spawn(async move {
        ...
        let body = r#"{"id":"resp_mapped","object":"response"}"#;
        let response = format!(
            "HTTP/1.1 200 OK\r\ncontent-length: {}\r\ncontent-type: application/json\r\nconnection: close\r\n\r\n{}",
            body.len(),
            body
    ...
    let mapped = RelayProfile {
        id: "mapped".to_string(),
        name: "Mapped".to_string(),
        base_url: format!("http://{addr}/v1"),
        upstream_base_url: format!("http://{addr}/v1"),
        api_key: "mapped-key".to_string(),
        relay_mode: RelayMode::PureApi,
        ..RelayProfile::default()
    };
    let settings = BackendSettings {
        hot_switch_enabled: true,
        hot_switch_relay_id: "fallback".to_string(),
        hot_switch_model: "fallback-model".to_string(),
        hot_switch_model_routing_enabled: true,
        hot_switch_model_mappings: vec![HotSwitchModelMapping {
            model: "codex-choice".to_string(),
            upstream_model: "upstream-real-model".to_string(),
            relay_id: "mapped".to_string(),
            candidate_relay_ids: vec!["mapped".to_string()],
            ..HotSwitchModelMapping::default()
        }],
        relay_profiles: vec![fallback, mapped],
        ..BackendSettings::default()
    };

    let result = open_responses_proxy_request_with_settings(
        r#"{"model":"codex-choice","input":"hi","stream":false}"#,
        settings,
    )
    .await
    .unwrap();
