...
async fn user_script_manager_deletes_market_script_metadata_and_rejects_builtin_delete() {
    ...
    let script = codex_plus_core::script_market::MarketScript {
        ...
        sha256: codex_plus_core::script_market::sha256_hex(b"window.demo = true;"),
    };
        ...
        "scripts": [
            {
                ...
                "script_url": "https://raw.githubusercontent.com/example/repo/main/demo.js",
...
#[test]
fn market_script_install_rolls_back_script_when_metadata_save_fails() {
    let temp = tempfile::tempdir().unwrap();
    let user_dir = temp.path().join("user");
    let invalid_config_path = temp.path().join("config-is-a-directory");
    std::fs::create_dir_all(&user_dir).unwrap();
    std::fs::create_dir_all(&invalid_config_path).unwrap();
    let script_path = user_dir.join("market-demo.js");
    std::fs::write(&script_path, "old").unwrap();
    let manager = UserScriptManager::new(
        temp.path().join("builtin"),
        user_dir.clone(),
        invalid_config_path,
    ...
    let script = codex_plus_core::script_market::MarketScript {
        ...
        homepage: String::new(),
        script_url: "https://raw.githubusercontent.com/example/repo/main/demo.js".to_string(),
        sha256: codex_plus_core::script_market::sha256_hex(b"new"),
    };
    ...
    assert_eq!(std::fs::read_to_string(&script_path).unwrap(), "old");
    assert_eq!(
        std::fs::read_dir(user_dir).unwrap().count(),
        1,
        "rollback must not leave a temporary or orphan script"
    );
    assert!(!temp.path().join("config-is-a-directory.tmp").exists());
}

#[test]
fn new_market_script_install_removes_new_file_when_metadata_save_fails() {
    let temp = tempfile::tempdir().unwrap();
    let user_dir = temp.path().join("user");
    let invalid_config_path = temp.path().join("config-is-a-directory");
    std::fs::create_dir_all(&user_dir).unwrap();
    ...
    let script = codex_plus_core::script_market::MarketScript {
        id: "new-demo".to_string(),
        name: "New Demo".to_string(),
        description: String::new(),
        version: "1.0.0".to_string(),
        author: String::new(),
        tags: Vec::new(),
        homepage: String::new(),
        script_url: "https://raw.githubusercontent.com/example/repo/main/new-demo.js".to_string(),
        sha256: codex_plus_core::script_market::sha256_hex(b"new"),
    };
    ...
    assert!(!user_dir.join("market-new-demo.js").exists());
    assert_eq!(std::fs::read_dir(user_dir).unwrap().count(), 0);
    assert!(!temp.path().join("config-is-a-directory.tmp").exists());
}

#[test]
fn user_script_delete_rolls_back_file_when_metadata_save_fails() {
    let temp = tempfile::tempdir().unwrap();
    let user_dir = temp.path().join("user");
    let invalid_config_path = temp.path().join("config-is-a-directory");
    std::fs::create_dir_all(&user_dir).unwrap();
    std::fs::create_dir_all(&invalid_config_path).unwrap();
    let script_path = user_dir.join("demo.js");
    ...
    assert!(manager.delete_user_script("user:demo.js").is_err());
    assert_eq!(std::fs::read_to_string(&script_path).unwrap(), "keep");
    assert_eq!(
        std::fs::read_dir(user_dir).unwrap().count(),
        1,
        "rollback must restore the staged script"
    );
    assert!(!temp.path().join("config-is-a-directory.tmp").exists());
}
