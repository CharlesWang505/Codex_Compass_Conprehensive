...
fn injection_script_prefixes_helper_url_and_version() {
    ...
    assert!(script.contains("window.__CODEX_PLUS_HELPER__"));
...
fn injection_script_keeps_bundled_marketplace_name_for_default_filter() {
    ...
    assert!(script.contains("if (name === \"openai-bundled\") return \"OpenAI插件1(Codex_Plus)\""));
}
...
fn injection_script_expands_api_key_plugin_marketplace_requests() {
    ...
    assert!(script.contains("if (name === \"openai-curated\") return \"OpenAI插件2(Codex_Plus)\""));
    assert!(
        script.contains(
    ...
    assert!(
        script
            .contains("if (name === \"openai-curated-remote\") return \"OpenAI插件5(Codex_Plus)\"")
    );
    assert!(script.contains(
        "if (name === \"codex-plus-openai-curated-remote\") return \"openai-curated-remote\""
    ));
    assert!(script.contains("OpenAI插件1(Codex_Plus)"));
    assert!(script.contains("OpenAI插件2(Codex_Plus)"));
    assert!(script.contains("OpenAI插件3(Codex_Plus)"));
