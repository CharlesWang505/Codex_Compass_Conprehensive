...
fn spawn_launcher_command_points_to_api_detect_hidden_watcher_mode() {
    ...
    assert!(!command.iter().any(|part| part.contains("codex_plus.exe")));
}
...
#[test]
fn codex_process_filter_accepts_renamed_chatgpt_executable_only_in_codex_package() {
    let processes = [
        (
            21,
...
fn launcher_process_filter_protects_current_process_ancestry() {
    let processes = [
        (10, 0, "codex_plus.exe"),
        (20, 10, "codex_plus.exe"),
        (30, 20, "codex_plus.exe"),
        (40, 10, "codex_plus.exe"),
        (50, 10, "codex-plus-plus.exe"),
    ];
...
#[cfg(windows)]
#[test]
fn find_codex_processes_ignores_standalone_chatgpt_process() {
    let processes = [WindowsProcessInfo {
        process_id: 46,
        parent_process_id: 0,
        exe_file: "ChatGPT.exe".to_string(),
...
fn find_codex_processes_ignores_unrelated_processes() {
    let processes = [
        ...
        WindowsProcessInfo {
            process_id: 20,
            parent_process_id: 0,
            exe_file: "codex_plus.exe".to_string(),
            executable_path: Some(std::path::PathBuf::from(
                r"D:\Programs\Codex_Plus\codex_plus.exe",
            )),
        },
    ];
