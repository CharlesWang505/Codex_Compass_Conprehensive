...
fn app_state_dir_override() -> Option<PathBuf> {
    APP_STATE_DIR_OVERRIDE
...
pub fn set_app_state_dir_override(path: Option<PathBuf>) -> Option<PathBuf> {
    APP_STATE_DIR_OVERRIDE
        .get_or_init(|| Mutex::new(None))
        .lock()
        .ok()
...
static APP_STATE_DIR_OVERRIDE: OnceLock<Mutex<Option<PathBuf>>> = OnceLock::new();
...
mod tests {
    ...
    fn default_settings_path_uses_app_state_directory() {
        ...
        assert!(path.ends_with(".codex_plus/settings.json"));
    }
    ...
    fn default_latest_status_path_uses_app_state_directory() {
        ...
        assert!(path.ends_with(".codex_plus/latest-status.json"));
    }
    ...
    fn default_diagnostic_log_path_uses_app_state_directory() {
        ...
        assert!(path.ends_with(".codex_plus/codex_plus.log"));
    }
    ...
    fn default_pending_provider_import_path_uses_app_state_directory() {
        ...
        assert!(path.ends_with(".codex_plus/pending-provider-import.json"));
    }
}
