...
impl BridgeSettingsService for CoreSettingsService {
    ...
    async fn codex_app_version(&self) -> anyhow::Result<String> {
        ...
        let app_dir = crate::app_paths::resolve_codex_app_dir_with_store(None, &self.store);
        Ok(app_dir
            .as_deref()
...
fn spawn_manager(manager_path: &Path) -> anyhow::Result<()> {
    let mut command = crate::windows_integration::silent_background_command(manager_path);
    command
        .spawn()
        .map(|_| ())
