...
use super::{
    SshTarget, remote_defaults_snapshot_script, remote_git_command, remote_path_join,
    resolve_ssh_target_from_global_state, shell_quote,
};
...
#[test]
fn managed_remote_connection_resolves_ssh_target_for_worktree() {
    let state = json!({
        "codex-managed-remote-connections": [{
            "hostId": "remote-ssh-codex-managed:remote",
            "sshHost": "longnv@100.125.101.8:2222"
        }]
    });

    let target =
        resolve_ssh_target_from_global_state(&state, "remote-ssh-codex-managed:remote").unwrap();
