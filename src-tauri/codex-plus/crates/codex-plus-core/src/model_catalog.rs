...
pub async fn read_codex_model_catalog() -> Value {
    ...
    let client = match crate::http_client::proxied_client("Codex_Plus/1.0") {
        Ok(client) => client,
        Err(error) => {
            return json!({
                "status": "failed",
...
pub async fn fetch_relay_profile_model_ids(
    ...
    if matches!(
        profile.protocol,
        crate::settings::RelayProtocol::Anthropic | crate::settings::RelayProtocol::Gemini
    ) {
        let base = source.base_url.trim_end_matches('/');
        let endpoint = match profile.protocol {
            crate::settings::RelayProtocol::Anthropic => {
            ...
            crate::settings::RelayProtocol::Gemini => {
                let base = if base.contains("/v1beta") || base.ends_with("/v1") {
                    base.to_string()
                } else {
                    format!("{base}/v1beta")
                };
                let key = url::form_urlencoded::byte_serialize(source.api_key.as_bytes())
                    .collect::<String>();
                format!("{base}/models?key={key}")
            }
            _ => unreachable!(),
        };
        ...
        let request = client
            .get(&endpoint)
            .header(reqwest::header::ACCEPT, "application/json");
        let request = if profile.protocol == crate::settings::RelayProtocol::Anthropic {
            request
        ...
        if !response.status().is_success() {
            anyhow::bail!("模型接口返回 HTTP {}", response.status().as_u16());
        }
        let mut models = unique_strings(parse_model_payload(&response.json::<Value>().await?));
        if profile.protocol == crate::settings::RelayProtocol::Gemini {
            models = unique_strings(
                models
                    .into_iter()
