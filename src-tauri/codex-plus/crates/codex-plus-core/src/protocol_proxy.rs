...
use crate::settings::{ReasoningDialect, RelayProtocol, SettingsStore};

pub const DEFAULT_PROTOCOL_PROXY_PORT: u16 = 58321;
pub const DEFAULT_HOT_SWITCH_PORT: u16 = 8787;
...
pub fn responses_to_chat_completions(body: Value) -> anyhow::Result<Value> {
    responses_to_chat_completions_with_dialect(body, ReasoningDialect::Inherit)
}

fn responses_to_chat_completions_with_dialect(
    body: Value,
    ...
    apply_chat_reasoning_options(&mut result, &body, model, reasoning_dialect);
...
pub fn upstream_http_client() -> anyhow::Result<reqwest::Client> {
    reqwest::Client::builder()
        ...
        .user_agent("Codex_Plus/ProtocolProxy")
        .build()
...
async fn open_responses_proxy_request_with_settings_and_user_agent(
    ...
    let (relays, mapped_model, upstream_model) = if let Some(route) = hot_switch_route {
        set_request_model(&mut request_json, &route.upstream_model);
        set_request_reasoning(&mut request_json, &route.reasoning);
        let mut relays = vec![route.relay];
        relays.extend(route.fallback_relays);
        (relays, route.mapped, route.upstream_model)
    } else {
        ...
        (relays, false, requested_model.clone())
    };
    ...
    for (attempt, relay) in relays.into_iter().enumerate() {
        ...
        if (200..300).contains(&status_code) || !has_more_candidates {
            let response_is_stream = match wire_api {
                UpstreamWireApi::Anthropic | UpstreamWireApi::Gemini => false,
                _ => is_stream || content_type.contains("text/event-stream"),
            };
...
pub async fn open_chat_completions_proxy_request(
    ...
    let mut request_json: Value = serde_json::from_str(body)?;
    let requested_model = request_model(&request_json);
    let route = crate::hot_switch_mapping::resolve_hot_switch_route(&settings, &requested_model)?;
    let relay = if let Some(route) = route {
        set_request_model(&mut request_json, &route.upstream_model);
        route.relay
    } else {
        settings.proxy_active_relay_profile()
    };
...
fn set_request_model(body: &mut Value, model: &str) {
...
fn set_request_reasoning(body: &mut Value, effort: &str) {
    ...
    if effort.is_empty() || effort == "auto" || effort == "inherit" {
        return;
    }
    let Some(object) = body.as_object_mut() else {
        return;
...
fn upstream_request_parts(
    ...
    match relay.protocol {
        ...
        RelayProtocol::ChatCompletions => Ok((
            ...
            responses_to_chat_completions_with_dialect(request_json, relay.reasoning_dialect)?,
            UpstreamWireApi::ChatCompletions,
        )),
        RelayProtocol::Anthropic => Ok((
            anthropic_messages_url(&relay.base_url),
            responses_to_anthropic(request_json, relay)?,
            UpstreamWireApi::Anthropic,
        )),
        RelayProtocol::Gemini => {
            let model = request_model(&request_json);
            Ok((
                gemini_generate_url(&relay.base_url, &model, relay.api_key.trim()),
                responses_to_gemini(request_json, relay)?,
                UpstreamWireApi::Gemini,
            ))
        }
    }
}
...
fn upstream_request_builder(
    ...
    builder = match wire_api {
        UpstreamWireApi::Anthropic => builder
            .header("x-api-key", relay.api_key.trim())
            .header("anthropic-version", "2023-06-01"),
        UpstreamWireApi::Gemini => builder,
        _ => builder.bearer_auth(relay.api_key.trim()),
    };
    if is_stream
        && !matches!(
            wire_api,
            UpstreamWireApi::Anthropic | UpstreamWireApi::Gemini
        )
    {
        builder = builder
...
fn responses_to_anthropic(
    ...
    let chat = responses_to_chat_completions_with_dialect(request, relay.reasoning_dialect)?;
    let record = chat
        .as_object()
        .ok_or_else(|| anyhow::anyhow!("转换后的 Chat 请求不是 JSON 对象"))?;
    let mut system = Vec::new();
    let mut messages = Vec::new();
    if let Some(items) = record.get("messages").and_then(Value::as_array) {
        for message in items {
            ...
            if role == "system" || role == "developer" {
                if let Some(text) = native_content_text(message.get("content")) {
                    if !text.is_empty() {
                        system.push(text);
                    }
                }
            ...
            if role == "tool" {
                messages.push(json!({
                    ...
                    "content": [{
                        "type": "tool_result",
                        "tool_use_id": message.get("tool_call_id").cloned().unwrap_or_else(|| json!("call_unknown")),
                        "content": native_content_text(message.get("content")).unwrap_or_default()
                    }]
                }));
                continue;
            }
    ...
    output.insert(
        ...
        record
            .get("model")
            .cloned()
            .unwrap_or_else(|| json!(relay.model)),
    );
    ...
    output.insert(
        ...
        record
            .get("max_tokens")
            .or_else(|| record.get("max_completion_tokens"))
            .cloned()
            .unwrap_or_else(|| json!(4096)),
    );
    output.insert("stream".into(), Value::Bool(false));
    if !system.is_empty() {
        output.insert("system".into(), Value::String(system.join("\n\n")));
    }
...
fn responses_to_gemini(
    ...
    if let Some(messages) = record.get("messages").and_then(Value::as_array) {
        for message in messages {
            ...
            if role == "tool" {
                parts.push(json!({
                    "functionResponse": {
                        "name": message.get("name").and_then(Value::as_str).unwrap_or("tool"),
                        "response": { "result": native_content_text(message.get("content")).unwrap_or_default() }
                    }
                }));
            } else {
                if let Some(text) = native_content_text(message.get("content")) {
    ...
    for (source, target) in [
        ...
        if let Some(value) = record.get(source) {
            generation.insert(target.into(), value.clone());
        }
    }
...
fn native_content_text(value: Option<&Value>) -> Option<String> {
    match value {
...
fn gemini_generate_url(base_url: &str, model: &str, api_key: &str) -> String {
    let base = base_url.trim().trim_end_matches('/');
    let base = if base.contains("/v1beta") || base.contains("/v1/") || base.ends_with("/v1") {
        base.to_string()
    } else {
        format!("{base}/v1beta")
    };
    let model = url::form_urlencoded::byte_serialize(model.as_bytes()).collect::<String>();
    let api_key = url::form_urlencoded::byte_serialize(api_key.as_bytes()).collect::<String>();
    format!("{base}/models/{model}:generateContent?key={api_key}")
}

pub fn upstream_json_to_response(
    wire_api: UpstreamWireApi,
    payload: Value,
    original_request: &Value,
) -> anyhow::Result<Value> {
    match wire_api {
        UpstreamWireApi::Responses => Ok(payload),
        UpstreamWireApi::ChatCompletions => {
            chat_completion_to_response_with_request(payload, original_request)
        }
        UpstreamWireApi::Anthropic => chat_completion_to_response_with_request(
            anthropic_to_chat_completion(payload, request_model(original_request)),
            original_request,
        ),
        UpstreamWireApi::Gemini => chat_completion_to_response_with_request(
...
fn anthropic_to_chat_completion(payload: Value, model: String) -> Value {
    ...
    json!({
        "id": payload.get("id").cloned().unwrap_or_else(|| json!(format!("msg_{}", uuid::Uuid::new_v4().simple()))),
...
fn gemini_to_chat_completion(payload: Value, model: String) -> Value {
    let mut text = String::new();
    let mut tool_calls = Vec::new();
    let parts = payload
        .get("candidates")
        ...
        for part in parts {
            ...
            if let Some(call) = part.get("functionCall") {
                tool_calls.push(json!({
                    "id": call.get("id").cloned().unwrap_or_else(|| json!(format!("call_{}", uuid::Uuid::new_v4().simple()))),
                    "type": "function",
                    "function": {
                        "name": call.get("name").cloned().unwrap_or_else(|| json!("tool")),
                        "arguments": call.get("args").map(Value::to_string).unwrap_or_else(|| "{}".to_string())
                    }
                }));
            }
        }
...
pub async fn handle_responses_proxy_request(body: &str) -> anyhow::Result<ProxyHttpResponse> {
    ...
    let response_json = upstream_json_to_response(wire_api, upstream_json, &request_json)?;
    if request_json
        .get("stream")
        .and_then(Value::as_bool)
        .unwrap_or(false)
        ...
        )
    {
        return Ok(ProxyHttpResponse {
            status: "200 OK".to_string(),
            content_type: "text/event-stream; charset=utf-8".to_string(),
            body: completed_response_to_sse(&response_json),
        });
    }
...
fn apply_chat_reasoning_options(
    ...
    let style = match dialect {
        ReasoningDialect::Inherit => infer_chat_reasoning_style(model),
        ReasoningDialect::Openai => ChatReasoningStyle::Default,
        ReasoningDialect::Openrouter => ChatReasoningStyle::OpenRouter,
        ReasoningDialect::Qwen | ReasoningDialect::Siliconflow => {
            ChatReasoningStyle::EnableThinking
        }
        ReasoningDialect::None => unreachable!(),
    };
    let force_openai = dialect == ReasoningDialect::Openai;
    ...
    match style {
        ...
        | ChatReasoningStyle::Default
            if force_openai || supports_reasoning_effort(model) =>
        {
...
mod hot_switch_tests {
    use super::*;
    use crate::settings::{BackendSettings, ReasoningDialect, RelayProfile};

    #[test]
    fn hot_switch_rewrite_only_replaces_model_field() {
        let settings = BackendSettings {
            hot_switch_enabled: true,
            hot_switch_model: "target-model".to_string(),
        ...
        let expected_reasoning = body["reasoning"].clone();

        let route = crate::hot_switch_mapping::resolve_hot_switch_route(
            &settings,
            body["model"].as_str().unwrap(),
        )
        .unwrap()
        .unwrap();
        set_request_model(&mut body, &route.upstream_model);

        assert_eq!(body["model"], json!("target-model"));
        assert_eq!(body["input"], expected_input);
        assert_eq!(body["reasoning"], expected_reasoning);
        assert_eq!(body["stream"], json!(true));
    }

    #[test]
    fn disabled_hot_switch_does_not_rewrite_model() {
        let settings = BackendSettings {
            hot_switch_enabled: false,
            hot_switch_model: "target-model".to_string(),
            ..BackendSettings::default()
        };
        let body = json!({ "model": "requested-model" });
    ...
    #[test]
    fn explicit_openrouter_reasoning_dialect_overrides_model_inference() {
        let converted = responses_to_chat_completions_with_dialect(
            json!({
                "model": "neutral-model",
                "input": "hello",
                "reasoning": { "effort": "high" }
        ...
        .unwrap();
        assert_eq!(converted["reasoning"]["effort"], json!("high"));
        assert!(converted.get("reasoning_effort").is_none());
    }

    #[test]
    fn anthropic_request_conversion_preserves_tools() {
        ...
        .unwrap();
        assert_eq!(converted["model"], json!("claude-test"));
        assert_eq!(converted["tools"][0]["name"], json!("lookup"));
        assert_eq!(converted["stream"], json!(false));
    }

    #[test]
    fn gemini_response_conversion_returns_responses_shape() {
        let request = json!({ "model": "gemini-test", "input": "hello", "stream": true });
        let converted = upstream_json_to_response(
            UpstreamWireApi::Gemini,
            json!({
        ...
        .unwrap();
        assert_eq!(converted["object"], json!("response"));
        assert_eq!(converted["output"][0]["content"][0]["text"], json!("world"));
