...
#[derive(Default)]
pub struct LocalHelperServer {
    runtime: Mutex<Option<HelperRuntime>>,
    endpoint: Mutex<Option<(String, u16)>>,
}

impl LocalHelperServer {
    pub async fn start(&self, bind_host: &str, port: u16) -> anyhow::Result<()> {
        ...
        let listener = match tokio::net::TcpListener::bind((bind_host, port)).await {
            ...
            Err(error)
                if error.kind() == std::io::ErrorKind::AddrInUse
                    && helper_runtime_healthy(bind_host, port).await =>
            {
                *self.endpoint.lock().await = Some((bind_host.to_string(), port));
                return Ok(());
            }
            Err(error) => {
                return Err(error).with_context(|| {
                    format!("failed to bind helper runtime on {bind_host}:{port}")
                });
            }
        };
        let _ = crate::diagnostic_log::append_diagnostic_log(
            ...
            serde_json::json!({
                "helper_port": port,
                "bind_host": bind_host,
                "address": format!("http://{bind_host}:{port}")
            }),
        );
    ...
    pub async fn is_running(&self) -> bool {
        ...
        match endpoint {
            Some((host, port)) => helper_runtime_healthy(&host, port).await,
            None => false,
        }
    }
}

async fn helper_runtime_healthy(bind_host: &str, port: u16) -> bool {
    let check = async {
        let mut stream = tokio::net::TcpStream::connect((bind_host, port))
            .await
            .ok()?;
        stream
        ...
        (response.starts_with("HTTP/1.1 200 OK") && response.contains("http-helper")).then_some(())
    };
    ...
    H: IntoLaunchHooks,
{
    ...
    let result: anyhow::Result<LaunchHandle> = async {
        ...
        if settings.hot_switch_enabled {
            helper_port = crate::protocol_proxy::DEFAULT_HOT_SWITCH_PORT;
        } else if protocol_proxy_enabled {
            helper_port = crate::protocol_proxy::DEFAULT_PROTOCOL_PROXY_PORT;
        }
        if settings.enhancements_enabled || protocol_proxy_enabled || settings.hot_switch_enabled {
        ...
        if settings.enhancements_enabled {
            ...
            if injection_ready {
                ...
                let degraded = launch_status(
                    "running_degraded",
                    "Codex launched; Codex_Plus enhancements are still waiting for the page bridge.",
                    debug_port,
                    helper_port,
                    &app_dir,
                );
        ...
        if !settings.enhancements_enabled || !injection_degraded {
            let status = launch_status(
                "running",
                "Codex_Plus embedded launch service ready",
                debug_port,
                helper_port,
                &app_dir,
            );
...
impl LaunchHooks for DefaultLaunchHooks {
    fn resolve_app_dir(
        ...
        crate::app_paths::resolve_codex_app_dir_with_store(app_dir, &SettingsStore::default())
    ...
    async fn start_helper(&self, helper_port: u16) -> anyhow::Result<()> {
        ...
        self.helper.start(&bind_host, helper_port).await
    }
...
async fn handle_helper_connection(
    ...
    let (status, body, content_type, log_event) = if matches!(path, "/backend/status" | "/health")
...
async fn handle_protocol_proxy_connection(
    ...
    let response_json = if let Some(request_json) = request_json.as_ref() {
        crate::protocol_proxy::upstream_json_to_response(wire_api, upstream_json, request_json)?
    } else {
        crate::protocol_proxy::upstream_json_to_response(
            wire_api,
            upstream_json,
            &serde_json::json!({}),
        )?
    };
    let client_requested_stream = request_json
        .as_ref()
    ...
    if client_requested_stream
        && matches!(
            wire_api,
            crate::protocol_proxy::UpstreamWireApi::Anthropic
                | crate::protocol_proxy::UpstreamWireApi::Gemini
        )
    {
        let body = crate::protocol_proxy::completed_response_to_sse(&response_json);
        write_http_response(stream, "200 OK", "text/event-stream; charset=utf-8", &body).await?;
        log_helper_response(
            "helper.protocol_proxy_native_stream_ok",
            method,
            path,
            "200 OK",
            remote_addr_text,
