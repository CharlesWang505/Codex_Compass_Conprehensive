...
pub fn apply_relay_config_to_home_preserving_existing_with_protocol(
    home: &Path,
    base_url: &str,
    bearer_token: &str,
    protocol: RelayProtocol,
    ...
    let existing = std::fs::read_to_string(home.join("config.toml")).unwrap_or_default();
    let codex_base_url = codex_base_url_for_protocol(base_url, protocol, proxy_port);
...
pub fn apply_hot_switch_config_to_home(
    home: &Path,
    base_url: &str,
    bearer_token: &str,
    catalog_entries: &[crate::model_suffix::ModelCatalogEntry],
    default_model: Option<&str>,
) -> anyhow::Result<RelayApplyResult> {
    const HOT_SWITCH_CATALOG: &str = "model-catalogs/hot-switch.json";
    let base_url = base_url.trim();
    if base_url.is_empty() {
        anyhow::bail!("热切换 Base URL 不能为空");
    }
    let bearer_token = bearer_token.trim();
    if bearer_token.is_empty() {
        anyhow::bail!("热切换 Key 不能为空");
    }
    let existing = std::fs::read_to_string(home.join("config.toml")).unwrap_or_default();
    let mut updated = upsert_model_provider_config(&existing, base_url, bearer_token)?;
    let mut doc = parse_toml_document(&updated)?;
    if catalog_entries.is_empty() {
        if doc
            .get("model_catalog_json")
            .and_then(Item::as_str)
            .is_some_and(|path| path == HOT_SWITCH_CATALOG)
        {
            doc.as_table_mut().remove("model_catalog_json");
        }
    } else {
        let catalog_path = home.join(HOT_SWITCH_CATALOG);
...
pub async fn test_relay_profile(
    ...
    let client = crate::http_client::proxied_client("Codex_Plus/RelayTest")?;
    let endpoint = match profile.protocol {
        ...
        RelayProtocol::Gemini => {
            format!("{base_url}/models/{test_model}:generateContent?key={api_key}")
        }
    };
    ...
    if http_status == 404 && !base_url.ends_with("/v1") {
        ...
        let v1_endpoint = match profile.protocol {
            ...
            RelayProtocol::Gemini => {
                format!("{v1_url}/models/{test_model}:generateContent?key={api_key}")
            }
        };
        let v1_request = client
            ...
            .header(reqwest::header::CONTENT_TYPE, "application/json")
            .json(&payload);
        let v1_request = match profile.protocol {
            RelayProtocol::Anthropic => v1_request
                .header("x-api-key", api_key)
                .header("anthropic-version", "2023-06-01"),
            RelayProtocol::Gemini => v1_request,
            _ => v1_request.bearer_auth(api_key),
        };
...
fn relay_profile_test_payload(protocol: RelayProtocol, model: &str) -> Value {
    match protocol {
        ...
        RelayProtocol::Gemini => serde_json::json!({
...
fn codex_base_url_for_protocol(base_url: &str, protocol: RelayProtocol, proxy_port: u16) -> String {
    match protocol {
        ...
        RelayProtocol::ChatCompletions | RelayProtocol::Anthropic | RelayProtocol::Gemini => {
