...
#[cfg(windows)]
pub fn windows_silent_background_command(program: &std::path::Path) -> std::process::Command {
    windows_integration::silent_background_command(program)
}
