...
pub const SILENT_NAME: &str = "Codex_Plus";
pub const MANAGER_NAME: &str = "Codex_Plus 管理工具";
pub const SILENT_BINARY: &str = "codex_plus";
...
pub fn shortcut_names() -> (&'static str, &'static str) {
    ("Codex_Plus.lnk", "Codex_Plus 管理工具.lnk")
}

pub fn app_bundle_names() -> (&'static str, &'static str) {
    ("Codex_Plus.app", "Codex_Plus 管理工具.app")
}
...
fn platform_install(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持安装 Codex_Plus 入口")
    }
}
...
fn platform_uninstall(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持卸载 Codex_Plus 入口")
    }
}
    ...
    if binary == SILENT_BINARY {
        ...
        return Some(
            macos
                ...
                .unwrap_or_else(|| macos.join("Codex_Plus")),
        );
    }
    if binary == MANAGER_BINARY {
        ...
        return Some(
            macos
                ...
                .unwrap_or_else(|| macos.join("Codex_Plus_Manager")),
        );
    }
    None
}
