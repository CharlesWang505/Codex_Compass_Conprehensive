...
const UNINSTALL_SUBKEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\Codex_Plus";
const URL_PROTOCOL_SUBKEY: &str = r"Software\Classes\codexplus";
...
pub fn build_windows_entrypoint_plan(options: &InstallOptions) -> WindowsEntrypointPlan {
    ...
    WindowsEntrypointPlan {
        ...
        uninstall_key: "Codex_Plus".to_string(),
...
pub fn install_shortcuts(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    register_url_protocol(&plan.launcher_path)?;
...
fn write_uninstall_registration(plan: &WindowsEntrypointPlan) -> anyhow::Result<()> {
    let install_location = Path::new(&plan.launcher_path)
        .parent()
        .map(Path::to_path_buf)
        ...
        .to_string();
    for (name, value) in [
        ("DisplayName", "Codex_Plus".to_string()),
...
fn register_url_protocol(manager_path: &str) -> anyhow::Result<()> {
    crate::windows_integration::set_current_user_string_value(
        URL_PROTOCOL_SUBKEY,
        "",
        "URL:Codex_Plus Import Protocol",
    )?;
    crate::windows_integration::set_current_user_string_value(
        URL_PROTOCOL_SUBKEY,
        "URL Protocol",
