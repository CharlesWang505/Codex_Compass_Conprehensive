...
pub fn run(options: LaunchOptions) -> Result<()> {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .context("failed to create embedded launcher runtime")?;
    runtime.block_on(run_async(options))
}

async fn run_async(options: LaunchOptions) -> Result<()> {
...
impl BridgeRuntimeService for LauncherRuntimeService {
    ...
    async fn open_manager(&self) -> anyhow::Result<Value> {
        ...
        #[cfg(windows)]
        {
            codex_plus_core::windows_silent_background_command(&manager_path)
                .spawn()
...
mod tests {
    ...
    #[test]
    fn injected_manager_action_resolves_unified_app_binary() {
