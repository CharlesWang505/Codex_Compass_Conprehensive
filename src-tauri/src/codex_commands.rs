...
pub struct HotSwitchRuntime {
    server: codex_plus_core::launcher::LocalHelperServer,
    operation: tokio::sync::Mutex<()>,
    last_error: tokio::sync::Mutex<Option<String>>,
}

impl Default for HotSwitchRuntime {
    fn default() -> Self {
        Self {
            server: codex_plus_core::launcher::LocalHelperServer::default(),
...
impl HotSwitchRuntime {
    pub async fn start_for_saved_settings(&self) -> anyhow::Result<()> {
        match self
            .server
            .start(
                "127.0.0.1",
                codex_plus_core::protocol_proxy::DEFAULT_HOT_SWITCH_PORT,
            )
            .await
        {
            Ok(()) => {
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct HotSwitchMappingPayload {
    pub settings: BackendSettings,
    pub settings_path: String,
    pub mappings: Vec<HotSwitchModelMapping>,
    pub providers: Vec<codex_plus_core::hot_switch_mapping::HotSwitchProviderScan>,
...
pub fn launch_codex_plus(request: LaunchRequest) -> CommandResult<Value> {
    spawn_codex_plus_launch(
        request,
        "启动任务已在当前应用后台开始，可稍后查看概览状态。",
        false,
    )
}
...
fn spawn_codex_plus_launch(
    ...
    match spawn_embedded_launcher(&request, wait_for_previous) {
        Ok(started) => CommandResult {
...
fn spawn_embedded_launcher(
    request: &LaunchRequest,
    wait_for_previous: bool,
) -> anyhow::Result<bool> {
    let mut slot = launcher_thread_slot()
        .lock()
        .map_err(|_| anyhow::anyhow!("内置启动服务状态锁已损坏"))?;
    if slot
        .handle
        .as_ref()
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let (event, detail) = match result {
                Ok(()) => (
                    "manager.embedded_launcher_stopped",
                    json!({ "status": "ok" }),
                ),
                Err(error) => (
                    "manager.embedded_launcher_failed",
                    json!({ "status": "failed", "error": error.to_string() }),
                ),
            };
            let _ = codex_plus_core::diagnostic_log::append_diagnostic_log(event, detail);
            if let Ok(mut slot) = launcher_thread_slot().lock() {
                slot.last_error = runtime_error;
            }
        })
...
#[derive(Default)]
struct LauncherThreadState {
    handle: Option<std::thread::JoinHandle<()>>,
    last_error: Option<String>,
}

fn launcher_thread_slot() -> &'static Mutex<LauncherThreadState> {
    static SLOT: OnceLock<Mutex<LauncherThreadState>> = OnceLock::new();
    SLOT.get_or_init(|| Mutex::new(LauncherThreadState::default()))
}

pub fn embedded_launcher_runtime_status() -> (bool, Option<String>) {
    launcher_thread_slot()
        .lock()
        .map(|slot| {
            (
                slot.handle
        ...
        .unwrap_or_else(|_| (false, Some("内置启动服务状态锁已损坏".to_string())))
}

pub fn start_hidden_watcher_launcher() -> anyhow::Result<bool> {
    if codex_plus_core::watcher::default_watcher_disabled_flag().exists() {
        return Ok(false);
    }
    let settings = SettingsStore::default().load().unwrap_or_default();
    spawn_embedded_launcher(
        &LaunchRequest {
...
fn preserve_detected_codex_app_path(current: &BackendSettings, next: &mut BackendSettings) {
    if next.codex_app_path.trim().is_empty()
        && codex_plus_core::app_paths::validated_codex_app_dir(Path::new(&current.codex_app_path))
            .is_some()
    {
        next.codex_app_path = current.codex_app_path.clone();
    }
}

#[tauri::command]
pub async fn hot_switch_status(
    runtime: tauri::State<'_, HotSwitchRuntime>,
) -> Result<CommandResult<HotSwitchPayload>, String> {
    Ok(hot_switch_status_inner(&runtime).await)
}

async fn hot_switch_status_inner(runtime: &HotSwitchRuntime) -> CommandResult<HotSwitchPayload> {
    let settings = SettingsStore::default().load().unwrap_or_default();
    let payload = hot_switch_payload(runtime, settings).await;
    if payload.enabled && !payload.running {
        failed("热切换配置为开启，但 8787 网关当前未运行。", payload)
    } else {
        ok("热切换状态已读取。", payload)
    }
}
...
#[tauri::command]
pub async fn set_hot_switch(
    runtime: tauri::State<'_, HotSwitchRuntime>,
    request: HotSwitchRequest,
) -> Result<CommandResult<HotSwitchPayload>, String> {
    Ok(set_hot_switch_inner(&runtime, request).await)
}

async fn set_hot_switch_inner(
    runtime: &HotSwitchRuntime,
    request: HotSwitchRequest,
) -> CommandResult<HotSwitchPayload> {
    let _operation = runtime.operation.lock().await;
    let store = SettingsStore::default();
    let current = store.load().unwrap_or_default();
    let mut next = current.clone();
    if !request.relay_id.trim().is_empty() {
        next.hot_switch_relay_id = request.relay_id.trim().to_string();
    }
    if !request.model.trim().is_empty() {
        next.hot_switch_model = request.model.trim().to_string();
    }
    next.hot_switch_enabled = request.enabled;
    next = normalize_settings_before_save(next);

    if request.enabled {
        let (profile, model) = match validate_hot_switch_target(&next) {
            Ok(target) => target,
            Err(error) => {
                return failed(
                    &format!("无法开启热切换：{error}"),
                    hot_switch_payload(runtime, current).await,
                );
            }
        };
        next.hot_switch_relay_id = profile.id.clone();
        next.hot_switch_model = model;

        if let Err(error) = runtime.start_for_saved_settings().await {
            return failed(
                &format!("启动 8787 热切换网关失败：{error}"),
                hot_switch_payload(runtime, current).await,
            );
        }
    ...
    if current.hot_switch_enabled {
        if let Err(error) = restore_direct_supplier_config(&current) {
            return failed(
                &format!("关闭热切换前恢复供应商配置失败：{error}"),
                hot_switch_payload(runtime, current).await,
            );
        }
    }
    ...
    *runtime.last_error.lock().await = None;
    ok(
        "8787 热切换网关已关闭，供应商配置已恢复可用。",
        hot_switch_payload(runtime, next).await,
    )
}
...
pub async fn scan_hot_switch_model_mappings(
    ...
    if scan.providers.is_empty() {
        return Ok(failed(
            ...
            hot_switch_mapping_payload(current, scan),
        ));
    }
    if scan.mappings.is_empty() {
        return Ok(failed(
            "所有供应商都未返回可用模型，已保留原模型映射。",
            hot_switch_mapping_payload(current, scan),
        ));
    }
    ...
    if !fallback_is_mapped {
        if let Some(mapping) = next.hot_switch_model_mappings.first() {
            next.hot_switch_relay_id = mapping.relay_id.clone();
            next.hot_switch_model = mapping.upstream_model.clone();
        }
    }
    next = normalize_settings_before_save(next);
    if let Err(error) = store.save(&next) {
        return Ok(failed(
            &format!("保存模型映射失败：{error}"),
            hot_switch_mapping_payload(current, scan),
        ));
    }
    if current.hot_switch_enabled {
        if let Err(error) = apply_hot_switch_codex_config(&next) {
            ...
            let _ = apply_hot_switch_codex_config(&current);
            return Ok(failed(
                &format!("写入 Codex 模型目录失败：{error}"),
                hot_switch_mapping_payload(current, scan),
            ));
        }
    }

    let payload = hot_switch_mapping_payload(next, scan.clone());
    let status = if scan.failed_provider_count > 0 || scan.conflict_count > 0 {
        "warning"
    } else {
        "ok"
    };
    let message = format!(
        "已生成 {} 条模型映射；{} 个重名模型，{} 个供应商扫描失败。",
        scan.mappings.len(),
        scan.conflict_count,
        scan.failed_provider_count
    );
    Ok(CommandResult {
...
#[tauri::command]
pub async fn save_hot_switch_model_mappings(
    runtime: tauri::State<'_, HotSwitchRuntime>,
    request: HotSwitchMappingsRequest,
) -> Result<CommandResult<HotSwitchMappingPayload>, String> {
    let _operation = runtime.operation.lock().await;
    let store = SettingsStore::default();
    let current = store.load().unwrap_or_default();
    ...
    if let Err(error) = store.save(&next) {
        return Ok(failed(
            &format!("保存模型映射失败：{error}"),
            hot_switch_mapping_payload(
                current,
                codex_plus_core::hot_switch_mapping::HotSwitchMappingScan {
                    mappings: Vec::new(),
                    providers: Vec::new(),
                    conflict_count: 0,
    ...
    let mappings = next.hot_switch_model_mappings.clone();
    let conflict_count = mappings
        .iter()
        .filter(|mapping| mapping.candidate_relay_ids.len() > 1)
        .count();
    Ok(ok(
        if next.hot_switch_model_routing_enabled {
            "Codex 模型自动路由已启用。"
        } else {
            "Codex 模型自动路由已关闭，恢复固定热切换目标。"
        },
        HotSwitchMappingPayload {
            settings: next,
            settings_path: codex_plus_core::paths::default_settings_path()
                .to_string_lossy()
                .to_string(),
...
fn provider_configuration_changed(current: &BackendSettings, next: &BackendSettings) -> bool {
    current.relay_profiles_enabled != next.relay_profiles_enabled
        || current.relay_base_url != next.relay_base_url
        || current.relay_api_key != next.relay_api_key
        || current.relay_profiles != next.relay_profiles
        || current.relay_common_config_contents != next.relay_common_config_contents
        || current.relay_context_config_contents != next.relay_context_config_contents
        || current.active_relay_id != next.active_relay_id
        || current.aggregate_relay_profiles != next.aggregate_relay_profiles
        || current.active_aggregate_relay_id != next.active_aggregate_relay_id
        || current.relay_test_model != next.relay_test_model
}

async fn hot_switch_payload(
    runtime: &HotSwitchRuntime,
    settings: BackendSettings,
) -> HotSwitchPayload {
    let profile = settings.hot_switch_relay_profile();
    let routing_enabled =
        settings.hot_switch_model_routing_enabled && !settings.hot_switch_model_mappings.is_empty();
    HotSwitchPayload {
        enabled: settings.hot_switch_enabled,
        running: runtime.server.is_running().await,
        base_url: codex_plus_core::protocol_proxy::local_responses_proxy_base_url(
            codex_plus_core::protocol_proxy::DEFAULT_HOT_SWITCH_PORT,
        ),
        relay_id: profile.id.clone(),
        relay_name: if routing_enabled {
            format!(
                "按模型自动路由（{} 条）",
                settings.hot_switch_model_mappings.len()
            )
        } else {
            profile.name.clone()
        },
        model: if routing_enabled {
            "由 Codex 当前模型决定".to_string()
        } else {
            settings.hot_switch_model.clone()
        },
        error: runtime.last_error.lock().await.clone(),
        settings,
        settings_path: codex_plus_core::paths::default_settings_path()
            .to_string_lossy()
            .to_string(),
...
fn hot_switch_mapping_payload(
    settings: BackendSettings,
    scan: codex_plus_core::hot_switch_mapping::HotSwitchMappingScan,
) -> HotSwitchMappingPayload {
    HotSwitchMappingPayload {
        mappings: scan.mappings,
        providers: scan.providers,
        conflict_count: scan.conflict_count,
        failed_provider_count: scan.failed_provider_count,
        settings,
        settings_path: codex_plus_core::paths::default_settings_path()
            .to_string_lossy()
            .to_string(),
...
fn validate_hot_switch_target(
    settings: &BackendSettings,
) -> anyhow::Result<(RelayProfile, String)> {
    let mut profile = settings.hot_switch_relay_profile();
    if profile.relay_mode == codex_plus_core::settings::RelayMode::Aggregate {
        anyhow::bail!("第一阶段暂不支持聚合供应商，请选择普通 API 供应商");
    }
    profile.base_url = codex_plus_core::relay_config::relay_profile_base_url(&profile);
    profile.api_key = codex_plus_core::relay_config::relay_profile_api_key(&profile);
    if profile.base_url.trim().is_empty() {
        anyhow::bail!("供应商「{}」缺少 Base URL", profile.name);
    }
    if profile.api_key.trim().is_empty() {
        anyhow::bail!("供应商「{}」缺少 API Key", profile.name);
    }
    let model = if !settings.hot_switch_model.trim().is_empty() {
        settings.hot_switch_model.trim().to_string()
    } else if settings.hot_switch_model_routing_enabled {
        settings
            .hot_switch_model_mappings
            .iter()
            .find(|mapping| mapping.relay_id == profile.id)
            .or_else(|| settings.hot_switch_model_mappings.first())
            .map(|mapping| mapping.upstream_model.clone())
            .unwrap_or_else(|| default_hot_switch_model(&profile))
    } else {
        default_hot_switch_model(&profile)
    };
    if model.is_empty() {
        anyhow::bail!(
            "供应商「{}」没有可切换模型，请先在供应商配置中填写模型",
            profile.name
        );
    }
    Ok((profile, model))
...
fn default_hot_switch_model(profile: &RelayProfile) -> String {
    let configured = codex_plus_core::relay_config::relay_profile_model(profile);
    if !configured.trim().is_empty() {
        return codex_plus_core::model_suffix::parse_model_suffix(&configured).0;
    }
    ...
    {
        return codex_plus_core::model_suffix::parse_model_suffix(model).0;
    }
    codex_plus_core::model_suffix::parse_model_suffix(profile.test_model.trim()).0
}

fn apply_hot_switch_codex_config(settings: &BackendSettings) -> anyhow::Result<()> {
    let home = codex_plus_core::relay_config::default_codex_home_dir();
    let entries = codex_plus_core::hot_switch_mapping::hot_switch_catalog_entries(settings);
    let default_model = codex_plus_core::hot_switch_mapping::hot_switch_default_model(settings);
    codex_plus_core::relay_config::apply_hot_switch_config_to_home(
        &home,
...
fn restore_direct_supplier_config(settings: &BackendSettings) -> anyhow::Result<()> {
    let home = codex_plus_core::relay_config::default_codex_home_dir();
    ...
    if relay_has_complete_files(&profile) {
...
pub fn enable_watcher() -> CommandResult<WatcherPayload> {
    ...
    let registration =
        codex_plus_core::watcher::watcher_registration_status(&launcher_path, default_debug_port());
    if !registration.valid {
        return failed(
            "启用 watcher 失败：请先重新安装 Watcher。",
            watcher_payload(),
        );
    }
    match codex_plus_core::watcher::enable_watcher().and_then(|_| {
        codex_plus_core::watcher::start_watcher(&launcher_path, default_debug_port())
...
pub fn save_relay_file(request: SaveRelayFileRequest) -> CommandResult<RelayFilesPayload> {
    ...
    {
        return failed(
            "热切换网关已开启，不能直接修改 Codex 配置文件。请先关闭热切换。",
...
pub fn switch_relay_profile(
    ...
    if current.hot_switch_enabled {
        ...
        return failed(
            "热切换网关已开启，不能执行供应商配置切换。请先关闭热切换。",
            relay_switch_payload(current, status, None),
        );
    }
    ...
    checks.push(ProviderDoctorCheck {
        ...
        detail: format!(
            ...
            match profile.protocol {
                ...
                codex_plus_core::settings::RelayProtocol::Gemini => "Gemini generateContent",
            }
        ),
    });
...
pub fn apply_pure_api_injection() -> CommandResult<RelayPayload> {
    ...
    if settings.hot_switch_enabled {
        let status = codex_plus_core::relay_config::relay_status_from_home(&home);
        return failed(
            "热切换网关已开启，不能直接写入纯 API 配置。请先关闭热切换。",
            relay_payload(status, None),
        );
    }
    if !settings.relay_profiles_enabled {
...
fn load_overview_payload() -> (
    ...
    (
        codex_plus_core::app_paths::resolve_codex_app_dir_with_store(
            None,
...
fn watcher_payload() -> WatcherPayload {
    ...
    let launcher_path = std::env::current_exe().unwrap_or_else(|_| PathBuf::new());
    let registration =
        codex_plus_core::watcher::watcher_registration_status(&launcher_path, default_debug_port());
    let runtime = codex_plus_core::watcher::watcher_runtime_status(&launcher_path);
    let (embedded_running, embedded_error) = embedded_launcher_runtime_status();
    let enabled = registration.installed && registration.valid && !flag.exists();
    ...
    WatcherPayload {
        ...
        enabled,
        running: enabled && (runtime.running || embedded_running),
        registration_valid: registration.valid,
        launcher_path: launcher_path.to_string_lossy().to_string(),
        process_id: runtime.process_id,
        last_error: embedded_error.or(runtime.last_error).or(registration_error),
...
mod tests {
    ...
    fn stale_frontend_settings_do_not_clear_a_persisted_detected_app_path() {
        ...
        let current = BackendSettings {
            codex_app_path: app.to_string_lossy().to_string(),
            ..BackendSettings::default()
        };
        let mut next = BackendSettings {
            relay_test_model: "keep-next-settings".to_string(),
            ..BackendSettings::default()
        };

        preserve_detected_codex_app_path(&current, &mut next);

        assert_eq!(next.codex_app_path, current.codex_app_path);
        assert_eq!(next.relay_test_model, "keep-next-settings");
    }
    ...
    #[test]
    fn user_scripts_live_under_unified_codex_state_directory() {
        assert_eq!(
            user_scripts_config_dir(),
            codex_plus_core::paths::default_app_state_dir().join("scripts")
        );
    }
...
"#
    ...
    #[test]
    fn hot_switch_fields_do_not_count_as_provider_configuration_changes() {
        let current = BackendSettings::default();
        let next = BackendSettings {
            hot_switch_enabled: true,
            hot_switch_relay_id: "default".to_string(),
            hot_switch_model: "gpt-5.4-mini".to_string(),
            ...
            hot_switch_model_mappings: vec![HotSwitchModelMapping {
                ...
                candidate_relay_ids: vec!["default".to_string()],
    ...
    #[test]
    fn hot_switch_target_uses_first_configured_model_when_selection_is_empty() {
        let profile = RelayProfile {
            id: "relay-a".to_string(),
            name: "供应商 A".to_string(),
        ...
        let (selected, model) = validate_hot_switch_target(&settings).unwrap();
